...
|name| is a framwork which facilitates the exchange of load and deformation data, and coordinates the separate aerodynamics and structure solvers. |name| performs a "*high-level* partitioned aeroelastic analyses". The actual aerodynamics and structure solvers have to be plugged into the framework. This is done using small wrapper modules, one for the CFD_ tool and one for the FEM_ tool. So to use |name| it is only necessary to write two wrapper modules.
...
The following pages describe the tool prerequisites, how wrappers can be set up and how |name| is used.
